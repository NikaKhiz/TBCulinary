...
async def fetch_recipe(session, recipe_url, base_url):
    async with session.get(recipe_url) as response:
        ...
        recipe_description = soup.find('div', class_='post__description').get_text().strip() or 'აღწერა არ არის დამატებული'
        ...
        recipe_stages = [
            {
                'step': recipe_stage.find('div').get_text(),
